...
app.post('/api/register', async (req, res) => {
  ...
  if (!/^[^@]+@[^@]+\.[^@]+$/.test(email)) return res.status(400).json({ error: '\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 email' });
  const db = await loadDB();
  if (db.users.find(u => u.username.toLowerCase() === username.toLowerCase())) return res.status(409).json({ error: '\u041B\u043E\u0433\u0438\u043D \u0437\u0430\u043D\u044F\u0442' });
  if (db.users.find(u => u.email && u.email.toLowerCase() === email.toLowerCase())) return res.status(409).json({ error: 'Email \u0443\u0436\u0435 \u0438\u0441\u043F\u043E\u043B\u044C\u0437\u0443\u0435\u0442\u0441\u044F' });
  const verifyCode = String(Math.floor(100000 + Math.random() * 900000));
  const u = { id: db.counter++, username, passwordHash: bcrypt.hashSync(password, 10), email, verified: false, verifyCode, role: 'user', hwid: null, license: { type: null, expiresAt: null, active: false }, balance: 0, createdAt: Date.now() };
  db.users.push(u);
  await saveDB(db);
  const token = jwt.sign({ id: u.id, username: u.username, role: u.role }, JWT_SECRET, { expiresIn: '30d' });
  res.json({ token, user: publicUser(u), needVerify: true, emailSent: false });
});
...
app.post('/api/login', async (req, res) => {
  ...
  if (!u.verified) return res.status(403).json({ error: 'Email \u043D\u0435 \u043F\u043E\u0434\u0442\u0432\u0435\u0440\u0436\u0434\u0435\u043D. \u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u043A\u043E\u0434.', needVerify: true });
  const token = jwt.sign({ id: u.id, username: u.username, role: u.role }, JWT_SECRET, { expiresIn: '30d' });
